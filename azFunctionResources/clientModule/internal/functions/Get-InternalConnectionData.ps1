...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSPrompt.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSPrompt.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSPrompt.Client.ProtectedToken'
