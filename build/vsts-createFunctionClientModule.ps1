...
$parentModule = 'PSPrompt'
if (-not $ModuleName) { $ModuleName = 'PSPrompt.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSPrompt'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSPrompt.psd1"
$prereqHash = @{
